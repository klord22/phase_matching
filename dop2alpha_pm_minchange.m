...
function dop2alpha_pm_minchange(file_name, nrec, amin)
...
r_gps_rec = sqrt((x_gps-x_rec).^2 + (y_gps-y_rec).^2 + (z_gps-z_rec).^2);
...
dist  = sqrt( (x_gps-x_rec).^2 + (y_gps-y_rec).^2 + (z_gps-z_rec).^2);
...
amax = amin+20;
a = amin:1e-3:amax;
[a_out, alpha, v] = pm_v2_air_minchange(time, ones(length(elev)-i_0+1,1), a, theta, r_gps, r_rec, phi, k, 1+nrec*1e-6, 1);
...
output_alpha = sprintf('Output_alpha_neg_%8s.txt',name_segment(5:12));

disp(output_alpha)  
disp(["file name", name_segment(5:12)])

output_v     = sprintf('Output_v_neg_%8s.txt',name_segment(5:12));
