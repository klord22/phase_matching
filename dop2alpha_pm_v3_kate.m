...
function dop2alpha_pm_v3_kate(file_name, nrec, amin, norp)
...
title(["Bending Angle vs Impact Parameter",name_segment(5:12)]);
xlabel("Alpha (rad)")
...
if norp == 1
    ...
    output_alpha = sprintf('Output_alpha_pos_%8s.txt',name_segment(5:12));
    output_v     = sprintf('Output_v_pos_%8s.txt',name_segment(5:12));
    file_alpha = fopen(output_alpha,'w');
    file_v     = fopen(output_v,'w');
end
